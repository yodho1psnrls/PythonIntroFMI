...
class QuadGrid(Mesh):
    def __init__(self, dim: glm.ivec2):
        self.dim = glm.ivec2(dim)
        # points = [
        #     Vertex(glm.vec3(uv.x, uv.y, 0), glm.vec3(0, 0, 1), uv) for uv in normal_uvs(dim.x, dim.y)
        # ],
        super().__init__(list(), quad_ids(dim.x, dim.y))
        for uv in normal_uvs(dim.x, dim.y):
            self.points.append(Vertex(glm.vec3(uv.x, uv.y, 0), glm.vec3(0, 0, 1), uv))
